...
class Disk(Problem):
    ...
    def __init__(self, initial, arr, resArr, goal=None):
    ...
    def successor(self, state):
        ...
        for i in range(state[0]):
            if i+1 < state[1] and self.arr[i+1] == 0 and self.arr[i] != 0:
                self.arr[i+1] = self.arr[i]
                self.arr[i] = 0
                succ[f"D1: Disk {i+1}"] = (state[0], state[1])

            elif i+2 < state[1] and i+1 != 0 and self.arr[i] != 0:
                self.arr[i+2] = self.arr[i]
                self.arr[i] = 0
                succ[f"D2: Disk {i+1}"] = (state[0], state[1])

            elif i > 0 and self.arr[i-1] == 0 and self.arr[i] != 0:
                self.arr[i-1] = self.arr[i]
                self.arr[i] = 0
                succ[f"L1: Disk {i+1}"] = (state[0], state[1])

            elif i > 1 and self.arr[i-1] !=0 and self.arr[i-2] == 0 and self.arr[i] != 0:
                self.arr[i-2] = self.arr[i]
                self.arr[i] = 0
                succ[f"L2: Disk {i+1}"] = (state[0], state[1])
        return succ
